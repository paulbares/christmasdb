...
public class TestQueryScenario {
  ...
  static void beforeAll() {
    ...
    fields.add(new Field("category", new FieldType(false, new ArrowType.Utf8(), null), null));
    ...
    List<Object[]> tuples = Arrays.asList(
            new Object[]{0, "syrup", "condiment", 2d},
            new Object[]{1, "tofu", "milk", 8d},
            new Object[]{2, "mozzarella", "milk", 4d}
    );
    datastore.load(Datastore.MAIN_SCENARIO_NAME, tuples);

    // Scenario 1
    List<Object[]> tuplesScenario1 = Arrays.asList(
            new Object[]{0, "syrup", "condiment", 3d},
            new Object[]{1, "tofu", "milk", 6d}
    );
    datastore.load("s1", tuplesScenario1);

    // Scenario 2
    List<Object[]> tuplesScenario2 = Arrays.asList(
            new Object[]{0, "syrup", "condiment", 4d},
            new Object[]{2, "mozzarella", "milk", 5d}
    );
  ...
  void testListThreeCoordinates() {
    ...
    Assertions.assertThat(result.getAggregates(List.of("s1", "tofu", "milk"))).containsExactly(6d);
    Assertions.assertThat(result.getAggregates(List.of("s1", "mozzarella", "milk"))).containsExactly(4d);
    Assertions.assertThat(result.getAggregates(List.of("s2", "tofu", "milk"))).containsExactly(8d);
    Assertions.assertThat(result.getAggregates(List.of("s2", "mozzarella", "milk"))).containsExactly(5d);
  }
